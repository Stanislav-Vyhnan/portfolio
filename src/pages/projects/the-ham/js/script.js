export const scriptFunc = () => {
  function gabmurgerFunc(gamburderSelector, navigationSelector, itemsSelector) {
    ...
    window.addEventListener('click', e => {
      if (
        ...
        e.target.classList[0] !== navigation.classList[0] &&
        e.target.classList[1] !== gamburger.classList[1] &&
        e.target.classList[0] !== 'gamburger__item'
      ) {
        gamburger.classList.remove('open');
        menuOpen = false;
        navigation.classList.remove('open');
  ...
  if (window.innerWidth >= 740) {
    ...
    function smallSlider() {
      ...
      const images = ['url(img/Home/fon1.jpg)', 'url(img/Home/fon2.jpg)', 'url(img/Home/fon3.jpg)'];
